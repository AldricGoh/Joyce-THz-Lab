...
class MCM3000:
	...
	x_axis = 0 # Channel 1 indexes at 0
	y_axis = 1 # Channel 2 indexes at 1

	def __init__(self, baud_rate=115200, data_bits=8, stopbits=1):
		'''Simply create a serial object.'''
		Parity = serial.PARITY_NONE
		stage = serial.Serial(port=None, baudrate=baud_rate, bytesize=data_bits, parity=Parity, stopbits=stopbits,timeout=0.1)
		self.serialobj = stage
	...
	def open(self, port):
		...
		The Port settings default to the correct values for the PLS. The only
		requires input parameter is the relevent COM port the stage is assigned. 
		'''
		parity = serial.PARITY_NONE
		self.serialobj = serial.Serial(port=port,
